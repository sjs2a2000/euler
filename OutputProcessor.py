...
class EulerProcessor(Processor):
    ...
    def add(self, x, y, orig_value):
        value = x+y if(orig_value is None) else max(x+y, orig_value)         
    ...
    def processInput(self, inputValues):
        ...
        for i, val in enumerate(inputValues):
            ...
            for index in indices: 
                inputValues[i] = self.add(self._results[index], x, compare)
                compare=inputValues[i]            
        return inputValues
